...
// Recuperarea valorilor de tip "double" din fisierul de configuratie
...
// Recuperarea valorilor de tip "string" din fisierul de configuratie
inline std::string get_string(const Config& cfg, const std::string& key) {
    auto item = cfg.find(key);
    if (item == cfg.end())
        throw std::invalid_argument("Input invalid: " + key);
    return item->second;
...
Stare make_stare(const Config& cfg) {
    ...
    // Temperaturi (deg C)
    s.T_target = get_double(cfg, "TEMPERATURA_TARGET");
...
Locatie make_locatie(const Config& cfg) {
    Locatie l;

    // Oras
    l.oras = get_string(cfg, "ORAS");

    // Data
    l.data = get_string(cfg, "DATA");
